...
router.get("/replyTicket/:id", ensureAuthenticated, async (req, res) => {
  ...
  const ResponseData = await Response.findAll({
    where: { ticketId: id },
    include: { model: TicketImages },
  });
  const ImageData = await TicketImages.findAll();

  for (let permitted of AdminUsers) {
    const userPermitted = await Permissions.findOne({
      where: { AdminName: permitted.dataValues.name, TicketID: id },
    });
    if (permitted.dataValues.name != req.user.dataValues.name) {
      if (!PermittedUsers[permitted.dataValues.name])
        PermittedUsers[permitted.dataValues.name] = {};
  ...
  Ticket.findOne({ where: { ticketId: id } }).then(async (data) => {
    ...
    var PermissionData = await Permissions.findOne({
      where: { AdminID: userId, ticketId: id },
    });
    ...
    if (PermissionData) {
      res.render("ticket/replyTicket", {
        TicketData: TicketData,
        ResponseData: ResponseData,
        user1: user1,
        TicketId: id,
        AdminUsers: PermittedUsers,
        PermissionData: PermissionData.id,
        ImageData: ImageData,
      });
    } else {
      res.render("ticket/replyTicket", {
...
router.post("/replyTicket/:id", async (req, res) => {
  const userId = req.user.dataValues.id;
  const TicketPermission = await Permissions.findOne({
    where: { adminID: userId, TicketID: req.params.id },
  });
  const TicketStatus = await Ticket.findOne({
    where: { ticketId: req.params.id },
  });
  const User = await Ticket.findAll({
    where: { ticketId: req.params.id, userId: userId },
  });
  ...
  if (TicketPermission || User) {
    if (TicketStatus.status == false) {
          ...
          if (req.files) {
            ...
            if (datalength == undefined) {
              ...
              blobStream.on("finish", async () => {
                let image = await TicketImages.create({
                  base64: date + req.files.imageFile.name,
      ...
      flashMessage(
        res,
        "error",
        "Ticket has been closed , this ticket is only for viewing"
      );
      res.redirect("back");
    }
  } else {
    flashMessage(
      res,
      "error",
      "You are not authorised to reply to this ticket , please request access from ticket master"
    );
    res.redirect("back");
  }
});
...
router.get("/takeUpTicket/:TicketId", async (req, res) => {
  ...
  let permission = Permissions.create({
    TicketID: TicketId,
    AdminID: userId,
    level: 1,
    AdminName: req.user.dataValues.name,
  });
  ...
  flashMessage(
    ...
    "Admin ID",
    userId,
    " has been assigned ticket number : ",
    TicketId
  );
  res.redirect("back");
});
...
router.get("/giveTicketPermission/:name", async (req, res) => {
  ...
  const checkPermission = await Permissions.findOne({
    where: { AdminID: id[5] },
  });
  ...
  if (checkPermission) {
    if (checkPermission.level == 1) {
      flashMessage(
        res,
        "error",
        "You cannot remove the permission of the ticket master"
      );
      res.redirect("back");
    } else {
...
router.get("/requestAccess", async (req, res) => {
  ...
  const TicketMaster = await Permissions.findOne({
    where: { level: 1, TicketID: id },
  });

  const TicketMasterUser = await User.findOne({
    where: { id: TicketMaster.AdminID },
  });
  ...
  sendRequest(TicketMasterUser.email, url, userId, id, name)
    .then((response) => {
      ...
      flashMessage(
        res,
        "success",
        "Please wait for the ticket master of ticket : " +
          id +
          " to grant you access"
      );
    ...
    .catch((err) => {
      ...
      flashMessage(
        res,
        "error",
        "Error when sending email to " + TicketMasterUser.email
      );
      res.redirect("/");
    });
});
...
router.get("/giveAccess/:id/:TicketId", async (req, res) => {
  ...
  if (user) {
    ...
    flashMessage(
      res,
      "success",
      `User ${user.name} has been granted access to ticket of ID ${TicketId}`
    );
    res.redirect("/");
  } else {
...
router.post("/completeTicket", async (req, res) => {
  try {
    ...
    const TicketMaster = await Permissions.findOne({
      where: { TicketID: TicketId, AdminID: req.user.dataValues.id },
    });
    if (reason) {
      Ticket.findOne({ where: { ticketId: TicketId } }).then(async (ticket) => {
        if (TicketMaster.level == 1) {
          if (ticket.assigned == true) {
            ...
            flashMessage(
              res,
              "success",
              "Ticket has been closed due to reason : " + reason
            );
            ...
            flashMessage(
              res,
              "error",
              "Ticket cannot be closed without being assigned"
            );
            res.redirect("back");
          }
        } else {
          flashMessage(
            res,
            "error",
            "Ticket cannot be closed by NON Ticket Master"
          );
          res.redirect("back");
        }
      });
...
router.get("/downloadPictures/:id", async (req, res, cb) => {
  ...
  for (let i = 0; i < Images.length; i++) {
    ...
    ImageInfoArray.push({
      url: `https://storage.googleapis.com/handyhandphone/${Images[i].base64}`,
      file: `download${i}.png`,
    });
  }
